...
contract('ICO changes to multivest', function (accounts) {
    ...
    it('create contract, buy tokens with correct sign address, get balance, check price', async function () {
        ...
        await instance.setSalePhases(
            ...
            preIcoSince,
            preIcoTill,
            new BigNumber("70000000000000").mul(100000000),
            0,
            0,
            new BigNumber("333333333333"),
            icoSince,
            icoTill,
            new BigNumber("200000000000000").mul(100000000)
        )
        // await instance.setWhitelistStatus(true)
            .then(() => Utils.balanceShouldEqualTo(instance, instance.address, new BigNumber("3000000000000000").mul(100000000).valueOf()))
            .then(function () {
            ...
            .then(() => Utils.balanceShouldEqualTo(instance, instance.address, new BigNumber("299999999699999999999700")))
            .then(function () {
    ...
    it('create contract, buy tokens, setMinMax contribution', async function () {
        ...
        await instance.setSalePhases(
            ...
            preIcoSince,
            preIcoTill,
            new BigNumber("10000000000000").mul(100000000),
            0,
            0,
            new BigNumber("333333333333"),
            icoSince,
            icoTill,
            new BigNumber("590000000000000").mul(100000000),
        )
    ...
    it('should not be able to buy without whitelist', async function () {
        ...
        await instance.setSalePhases(
            ...
            new BigNumber("10000000000000").mul(100000000), //_allocatedTokensForPreICO
            0, //_minPreIcoContribution
            0, //_maxPreIcoContribution
            new BigNumber("333333333333"), //_icoTokenPrice
            icoSince, //_icoSince
            icoTill, //_icoTill
            new BigNumber("590000000000000").mul(100000000), //_goalIcoMinSoldTokens
        )
        await instance.setAllowedMultivest(accounts[0])
        await instance.setWhitelistStatus(true)
        await instance.addToWhitelist(0, accounts[0])
    ...
    it('create contract, set new Price', async function () {
        var icoSince = parseInt(new Date().getTime() / 1000)+ 3600*2;
        var icoTill = parseInt(new Date().getTime() / 1000) + 3600*3;
        var preIcoSince = parseInt(new Date().getTime() / 1000 - 200);
        var preIcoTill = parseInt(new Date().getTime() / 1000) + 3600;
        ...
        await instance.setMinMaxContribution(0, web3.toWei('2', 'ether'),  web3.toWei('3', 'ether'))
            .then(() => instance.allowedMultivests.call(accounts[0]))
            .then((result) => assert.equal(result.valueOf(), true, "should be true"))
        await instance.setTokenPrice(0, new BigNumber("111111111111"))
        await instance.multivestBuy(accounts[0], web3.toWei('2', 'ether').valueOf())
            .then(Utils.receiptShouldSucceed)
            //2*10^18 * (10 ^ 8) / 111111111111
            .then(() => Utils.balanceShouldEqualTo(instance, accounts[0], new BigNumber("1800000000001800").valueOf()))
    });
    ...
    it('create contract, setICOFinished', async function () {
        var icoSince = parseInt(new Date().getTime() / 1000) - 3600*2;
        var icoTill = parseInt(new Date().getTime() / 1000) + 3600*3;
        ...
        await instance.multivestBuy(accounts[0], web3.toWei('2', 'ether').valueOf())
            .then(Utils.receiptShouldSucceed)
            //2*10^18 * (10 ^ 8) / 333333333333
            .then(() => Utils.balanceShouldEqualTo(instance, accounts[0], new BigNumber("750000000000750").valueOf()))
    ...
    it('create contract, refund', async function () {
        ...
        await instance.multivestBuy(accounts[0], web3.toWei('2', 'ether').valueOf())
            ...
            //2*10^18 * (10 ^ 8) / 333333333333
            .then(() => Utils.balanceShouldEqualTo(instance, accounts[0], new BigNumber("750000000000750").valueOf()))
        await instance.sendTransaction({from:accounts[3], value:web3.toWei('2', 'ether').valueOf()})
            .then(Utils.receiptShouldSucceed)
    ...
    it('create contract, not enough tokens to buy', async function () {
        ...
        await instance.setSalePhases(
            ...
            preIcoSince,
            preIcoTill,
            new BigNumber("2700000000002700"),
            0,
            0,
            new BigNumber("333333333333"),
            icoSince,
        ...
        await instance.multivestBuy(accounts[0], web3.toWei('2', 'ether').valueOf())
            ...
            //2*10^18 * (10 ^ 8) / 111111111111
            .then(() => Utils.balanceShouldEqualTo(instance, accounts[0], new BigNumber("1800000000001800").valueOf()))
        await instance.multivestBuy(accounts[0], web3.toWei('2.5', 'ether').valueOf())
            .then(Utils.receiptShouldFailed)
            .catch(Utils.catchReceiptShouldFailed)
        await instance.transferEthers()
    });
